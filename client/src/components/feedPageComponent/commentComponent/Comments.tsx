...
import replyIcon from '../../../assets/mainPageImages/postContainerIcons/reply.png';
...
const Comments = ({ comment }: { comment: CommentType}) => {
    ...
    const likeComment = async () => {
        try {
            await postService.toggleCommentLike(comment._id, isLiked);
    ...
    return(
        <div className="mx-2 md:mx-10 py-2 md:py-3 border-b border-gray-200 dark:border-gray-700 relative">
            ...
            <div className="flex gap-3 items-center relative">
                ...
                {isMenuOpen && (
                    <div ref={menuRef} className="absolute top-full right-0 mt-2 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg shadow-lg w-40 overflow-hidden z-10">
                        {
                            comment.isAuthor && (
                                <button onClick={() => deleteComment()} className="flex items-center gap-2 w-full px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
            ...
            <div className="ml-10 md:ml-12 mt-1 md:mt-3">
                ...
                {/* Meta Info */}
                <div className="flex gap-4 text-xs text-gray-500 dark:text-gray-400 mt-2 ml-1">
                <span>{noOfLikes} Likes</span>
                <span>{comment.noOfReplies} Replies</span>
                </div>
                ...
                <div className="flex gap-3 mt-1 md:mt-2">
                    ...
                    <button onClick={() => setReplySection(!replySection)} className="flex items-center gap-1 px-3 py-1 rounded-md text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                        <img src={replyIcon} alt="" className="w-4 h-4 dark:invert" />
                        Reply
                    </button>
                </div>
            </div>
            ...
            {
                replySection && <ReplySection postId={comment.postId} commentId={comment._id} noOfReplies={comment.noOfReplies} />
            }

            {
                isCommentDeleted && (
                    <div className="bg-white dark:bg-slate-950 absolute top-0 left-0 w-full h-full flex justify-center items-center">
                        <p className="font-bold dark:text-white">COMMENT DELETED!!</p>
                    </div>
                )
            }
