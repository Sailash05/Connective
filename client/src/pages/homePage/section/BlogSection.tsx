...
const BlogSection = ({ id }: { id: string }) => {
    return (
        <section id={id} className="scroll-mt-12 md:scroll-mt-20 py-6 w-11/12 max-w-6xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-center mb-3 md:mb-6">From Our Blog</h2>
            <p className="text-gray-500 text-center max-w-xl mx-auto mb-6 md:mb-12">
            ...
            <div className="grid md:grid-cols-3 gap-4 md:gap-8">
                {
                    blogPosts.map((post, index) => (
                        <a href={post.link} key={index} className="bg-white rounded-xl shadow-md hover:shadow-lg p-5 block hover:scale-[1.05] transition-all duration-300">
